...
- The YubiKey LUKS salt rotation feature has been removed from `boot.initrd.luks.devices.<name>.yubikey`.
  ...
  New options for static salt configuration:
  - `boot.initrd.luks.devices.<name>.yubikey.salt` - embed salt directly in initramfs
  - `boot.initrd.luks.devices.<name>.yubikey.iterations` - PBKDF2 iteration count

  Existing users must re-enroll their LUKS keys with a static salt. Generate a new salt with
  `dd if=/dev/random bs=1 count=16 | od -An -vtx1 | tr -d ' \n'` and use `cryptsetup luksChangeKey`
  to update your key slot with the new derived key.
